...
class ProfileAgent:
    def run(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        course = state.get("course") or "未指定学科"
        return {
    ...
    @staticmethod
    def _extract_weak_points(message: str) -> list[str]:
        candidates = [
            "函数依赖", "候选码", "范式判断", "关系模型", "SQL", "事务",
            "并发控制", "索引", "存储管理", "数据结构", "算法", "进程调度",
            "内存管理", "计算机网络", "软件工程",
        ]
        found = [item for item in candidates if item in message]
        return found or ["需要通过后续学习行为进一步识别"]
    ...
    def _extract_preferences(message: str) -> list[str]:
        ...
        return preferences or ["讲解文档", "练习题"]
