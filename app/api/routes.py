...
from app.learning.llm import call_llm, stream_llm
        ...
        try:
            ...
            if request.long_task_mode:
                ...
                if not successful_reports:
                    raise ValueError("长任务模式的所有 Sub-Agent 均执行失败，请检查模型接口配置")
                reports.sort(key=lambda item: item["id"])
                state["long_task_reports"] = reports
                report_context = "\n\n".join(
                    f"## {item['agent']}：{item['task']}\n{item['report']}"
                    for item in reports
                )
                state["source_context"] = (
                    f"{state.get('source_context', '')}\n\n"
                    f"以下是主控 Agent 调用并行 Sub-Agent 获得的阶段报告：\n{report_context}"
                ).strip()
                yield _sse("status", {
                    "message": f"已接收 {len(successful_reports)}/{len(specs)} 份阶段报告",
                    "agent": "主控 Agent",
...
COURSE_CATALOG: list[dict[str, Any]] = [
    {
        ...
        "name": "数据库系统",
        "icon": "DB",
        "description": "配套《数据库系统概论》第 5 版课件，覆盖关系模型、SQL、数据库设计、恢复技术与并发控制。",
        "progress": 50,
        "totalHours": 48,
        "completedHours": 24,
        "status": "in-progress",
        ...
        "difficulty": "中等",
        "chapters": [
            {"id": "db-01", "name": "绪论", "hours": 3, "status": "completed", "topics": ["数据模型", "数据库系统结构"]},
            {"id": "db-02", "name": "关系数据库", "hours": 4, "status": "completed", "topics": ["关系模型", "关系代数", "关系完整性"]},
            {"id": "db-03", "name": "关系数据库标准语言 SQL", "hours": 6, "status": "current", "topics": ["SELECT", "数据更新", "视图"]},
            {"id": "db-04", "name": "数据库安全性", "hours": 4, "status": "pending", "topics": ["授权", "访问控制", "审计"]},
            {"id": "db-05", "name": "数据库完整性", "hours": 4, "status": "pending", "topics": ["实体完整性", "参照完整性", "触发器"]},
            {"id": "db-06", "name": "关系数据理论", "hours": 5, "status": "pending", "topics": ["函数依赖", "范式", "数据依赖公理"]},
            {"id": "db-07", "name": "数据库设计", "hours": 6, "status": "pending", "topics": ["需求分析", "概念结构", "逻辑结构"]},
            {"id": "db-08", "name": "数据库编程", "hours": 4, "status": "pending", "topics": ["嵌入式 SQL", "存储过程", "ODBC"]},
            {"id": "db-09", "name": "关系查询处理和查询优化", "hours": 4, "status": "pending", "topics": ["查询处理", "代数优化", "物理优化"]},
            {"id": "db-10", "name": "数据库恢复技术", "hours": 4, "status": "pending", "topics": ["事务", "日志", "故障恢复"]},
            {"id": "db-11", "name": "并发控制", "hours": 4, "status": "pending", "topics": ["封锁", "可串行化", "两段锁协议"]},
        ],
