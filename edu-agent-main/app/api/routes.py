...
综合评估：您的整体表现{'优秀' if avg_score >= 80 else '良好' if avg_score >= 60 else '需加强'}，建议重点复习薄弱环节。""",
        ...
        "recommended_resources": _generate_recommended_resources(
            user_id=request.user_id,
            course=request.course,
            weak_points=weak_points + weak_topics,
            accuracy=avg_score
...
def finish_quiz(request: SmartEvaluateRequest) -> dict:
    ...
    # 自动生成推荐资源（评估→调整闭环）
    result["recommended_resources"] = _generate_recommended_resources(
        user_id=request.user_id,
        course=quiz["course"],
        weak_points=weak_points,
        accuracy=accuracy
...
def _generate_recommended_resources(
    *, user_id: str, course: str, weak_points: list[str], accuracy: float
) -> list[dict[str, Any]]:
    """根据画像的资源偏好和易错点生成推荐资源"""
    profile = profile_service.get_profile(user_id, course)
    dimensions = profile.get("dimensions", {})
    
    # 获取资源偏好
    resource_preferences = dimensions.get("资源偏好", {}).get("value", [])
    if not resource_preferences:
        resource_preferences = ["讲解文档", "练习题"]
    
    # 获取知识基础
    knowledge_level = dimensions.get("知识基础", {}).get("value", "中等")
    
    # 获取认知风格
    learning_style = dimensions.get("认知风格", {}).get("value", "概念讲解结合练习")
    ...
    # 资源类型映射
    resource_type_map = {
        "讲解文档": {"icon": "📖", "type": "document", "description": "详细讲解文档"},
        "思维导图": {"icon": "🧠", "type": "mindmap", "description": "知识结构导图"},
        "练习题": {"icon": "✏️", "type": "exercise", "description": "分层练习题"},
        "案例实践": {"icon": "💻", "type": "practice", "description": "实战案例"},
        "教学视频": {"icon": "🎬", "type": "video", "description": "教学视频"},
    }
    
    # 为每个薄弱点生成推荐
    for idx, weak_point in enumerate(weak_points[:5]):
        # 根据薄弱程度确定优先级
        priority = "high" if accuracy < 60 else "medium" if accuracy < 80 else "low"
        
        # 根据认知风格选择最合适的资源类型
        if "步骤化" in learning_style:
            preferred_types = ["讲解文档", "练习题"]
        elif "图示" in learning_style or "视觉" in learning_style:
            preferred_types = ["思维导图", "讲解文档"]
        elif "实践" in learning_style:
            preferred_types = ["案例实践", "练习题"]
        else:
            preferred_types = resource_preferences
        
        # 为每个薄弱点推荐2-3种资源类型
        for pref in preferred_types[:3]:
            resource_info = resource_type_map.get(pref, resource_type_map["讲解文档"])
            
            recommended.append({
                "id": f"rec-{user_id}-{course}-{weak_point}-{pref}-{idx}",
                "topic": weak_point,
                "resource_type": pref,
                "icon": resource_info["icon"],
                "description": f"{resource_info['description']} - {weak_point}",
                "title": f"{resource_info['icon']} {weak_point} - {pref}",
                "priority": priority,
                "estimated_time": "20-45分钟",
                "course": course,
                "reason": f"基于您在{weak_point}知识点的薄弱表现，推荐通过{pref}进行专项提升",
            })
    
    # 如果没有薄弱点，推荐一个综合性的进阶资源
    if not weak_points:
        first_pref = resource_preferences[0] if resource_preferences else "讲解文档"
        resource_info = resource_type_map.get(first_pref, resource_type_map["讲解文档"])
        recommended.append({
            "id": f"rec-{user_id}-{course}-advanced",
            "topic": "进阶学习",
            "resource_type": first_pref,
            "icon": resource_info["icon"],
            "description": f"{resource_info['description']} - 进阶内容",
            "title": f"{resource_info['icon']} 进阶{course}学习",
            "priority": "low",
            "estimated_time": "30-60分钟",
            "course": course,
            "reason": "您已掌握基础知识点，推荐进阶学习内容",
        })
...
def get_recommendations(user_id: str, course: str | None = None) -> dict:
    """获取基于画像的个性化资源推荐"""
    if course:
        profile = profile_service.get_profile(user_id, course)
        ...
        # 获取最新评估正确率
        stats = stats_service.get_course_stats(user_id, course)
        accuracy = stats.get("correct_rate", 0)
        
        resources = _generate_recommended_resources(
            user_id=user_id,
            course=course,
            weak_points=weak_points,
            accuracy=accuracy
        ...
        for p in profiles[:3]:
            c = p.get("course", "")
            if c:
                weak_points = p.get("dimensions", {}).get("易错点", {}).get("value", [])
                stats = stats_service.get_course_stats(user_id, c)
                accuracy = stats.get("correct_rate", 0)
                resources.extend(_generate_recommended_resources(
                    user_id=user_id,
                    course=c,
                    weak_points=weak_points,
                    accuracy=accuracy
    ...
    resources.sort(key=lambda x: priority_order.get(x["priority"], 2))
    
    return {
        "recommendations": resources[:10],
        "total": len(resources),
        "user_id": user_id,
    }
...
def generate_recommendation_content(request: CollaborativeLearningRequest) -> dict:
    """根据推荐生成具体学习内容（讲解文档、思维导图、练习题）"""
    ...
    try:
        # 使用LLM生成讲解文档
        lecture_prompt = f"""你是一位专业的计算机学科教师。请针对课程【{request.course}】中的知识点【{request.chapter}】，生成一份详细的讲解文档。
...
请开始生成讲解文档："""
        lecture_doc = llm_client.generate(lecture_prompt, task="document")
        
        # 使用LLM生成思维导图
        mindmap_prompt = f"""你是一位专业的知识结构梳理专家。请针对课程【{request.course}】中的知识点【{request.chapter}】，生成一份思维导图。

要求：
1. 使用Markdown思维导图语法（mindmap格式）
2. 包含核心概念、关键原理、应用场景、相关知识点
3. 结构清晰，层次分明

请开始生成思维导图："""
        mindmap = llm_client.generate(mindmap_prompt, task="mindmap")
        
        # 使用LLM生成练习题
        exercise_prompt = f"""你是一位专业的计算机学科出题专家。请针对课程【{request.course}】中的知识点【{request.chapter}】，生成3道练习题。

要求：
1. 包含选择题、填空题、问答题各1道
2. 每道题包含：题目、选项（选择题）、答案、详细解析
3. 难度适中，适合进阶学习者
4. 输出格式为JSON数组，每个对象包含：question(题目), options(选项数组，填空题和问答题可为空数组), answer(答案), explanation(解析)

请开始生成练习题（只输出JSON，不要包含其他文字）："""
        
        exercise_result = llm_client.generate(exercise_prompt, task="general")
        
        # 解析练习题JSON
        try:
            # 提取JSON部分
            import re
            json_match = re.search(r'\[.*\]', exercise_result, re.DOTALL)
            if json_match:
                exercise_items = json.loads(json_match.group())
            else:
                # 如果不是JSON格式，使用默认练习题
            ...
            exercise_items = [
                {
                    "question": f"关于{request.chapter}，以下哪个描述是正确的？",
                    "options": ["A. 选项A", "B. 选项B", "C. 选项C", "D. 选项D"],
                    "answer": "B",
                    "explanation": "正确答案是B，因为..."
                },
                {
                    "question": f"{request.chapter}的核心特点是什么？",
                    "options": [],
                    "answer": "答案",
                    "explanation": "解析..."
                },
                {
                    "question": f"请简述{request.chapter}的工作原理及其应用场景。",
                    "options": [],
                    "answer": "答案",
                    "explanation": "解析..."
                }
        ...
        # 生成练习题Markdown
        exercises_markdown = f"## {request.chapter}练习题\n\n"
        for i, item in enumerate(exercise_items, 1):
            exercises_markdown += f"### 第{i}题\n\n"
            exercises_markdown += f"**题目**：{item.get('question', '')}\n\n"
            if item.get('options'):
                for opt in item['options']:
                    exercises_markdown += f"- {opt}\n"
                exercises_markdown += "\n"
            exercises_markdown += f"**答案**：{item.get('answer', '')}\n\n"
            exercises_markdown += f"**解析**：{item.get('explanation', '')}\n\n"
        
        # 构建结果
        result = {
            "lectureDoc": lecture_doc,
            "mindmap": mindmap,
            "exercises": exercises_markdown,
        ...
        # 保存生成的资源
        resources = []
        if result.get("lectureDoc"):
            resource = resource_service.save_generated_resource(
                user_id=request.user_id,
                name=f"{request.chapter} - 讲解文档",
                content=result["lectureDoc"],
                resource_type="lecture",
                course_folder=request.course,
            )
        ...
        if result.get("exerciseItems"):
            from app.learning.agents import _exercises_to_markdown
            exercises_content = _exercises_to_markdown({}, result["exerciseItems"])
            resource = resource_service.save_generated_resource(
                user_id=request.user_id,
                name=f"{request.chapter} - 练习题",
        ...
        # 捕获所有异常，返回默认学习内容
        import traceback
        print(f"Error generating recommendation content: {exc}")
        print(traceback.format_exc())
        
        # 返回默认学习内容
        default_result = {
            "lectureDoc": f"## {request.chapter} 知识点讲解\n\n本节将详细讲解{request.chapter}相关知识。\n\n### 核心概念\n\n{request.chapter}是{request.course}中的重要知识点，涉及以下核心概念：\n\n1. **基础概念**：了解{request.chapter}的基本定义和作用\n2. **核心原理**：掌握{request.chapter}的工作原理\n3. **应用场景**：学习{request.chapter}的实际应用\n\n### 学习建议\n\n- 先理解基本概念\n- 结合实际例子加深理解\n- 通过练习巩固知识",
            "mindmap": f"mindmap\n  root(({request.chapter}))\n    核心概念\n      定义\n      特点\n      分类\n    核心原理\n      原理1\n      原理2\n      原理3\n    应用场景\n      场景1\n      场景2\n    相关知识点\n      知识点1\n      知识点2",
            "exercises": f"## {request.chapter}练习题\n\n### 选择题\n\n1. 关于{request.chapter}，以下说法正确的是？\n   A. 选项A\n   B. 选项B\n   C. 选项C\n   D. 选项D\n\n**答案**：B\n**解析**：正确答案是B。\n\n### 填空题\n\n2. {request.chapter}的核心特点是______。\n\n**答案**：答案\n**解析**：解析内容。\n\n### 问答题\n\n3. 请简述{request.chapter}的工作原理。\n\n**答案**：答案\n**解析**：解析内容。",
            "exerciseItems": [
                {
                    "question": f"关于{request.chapter}，以下哪个描述是正确的？",
                    "options": ["A. 选项A", "B. 选项B", "C. 选项C", "D. 选项D"],
