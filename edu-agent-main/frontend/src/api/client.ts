...
export async function getRecommendations(userId: string, course?: string): Promise<{ recommendations: RecommendedResource[]; total: number }> {
  const params = course ? `?user_id=${encodeURIComponent(userId)}&course=${encodeURIComponent(course)}` : `?user_id=${encodeURIComponent(userId)}`
  return httpRequest(`${API_BASE}/recommendations${params}`)
}

export async function generateRecommendationContent(payload: CollaborativeLearningRequest): Promise<any> {
  return httpRequest(`${API_BASE}/recommendations/generate`, {
    method: 'POST',
    body: JSON.stringify(payload)
  })
}
