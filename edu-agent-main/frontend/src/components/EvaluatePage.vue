...
const mockSmartResult = {
  ...
  recommended_resources: [
    { id: '1', topic: '函数依赖', resource_type: 'video', icon: '🎬', description: '详细讲解函数依赖的概念和应用', title: '函数依赖深度解析', priority: 'high', estimated_time: '45分钟', course: '数据库系统', reason: '您在函数依赖相关题目上表现较弱，建议重点学习' },
    { id: '2', topic: '范式判断', resource_type: 'practice', icon: '✏️', description: '包含大量范式判断练习题', title: '范式判断专项练习', priority: 'high', estimated_time: '60分钟', course: '数据库系统', reason: '范式判断是您的薄弱环节，通过练习加深理解' },
    { id: '3', topic: '关系模型', resource_type: 'review', icon: '📖', description: '关系模型知识点复习', title: '关系模型知识点回顾', priority: 'medium', estimated_time: '30分钟', course: '数据库系统', reason: '巩固已掌握的知识点' },
    { id: '4', topic: 'SQL进阶', resource_type: 'video', icon: '🎬', description: 'SQL高级查询技巧', title: 'SQL进阶教程', priority: 'low', estimated_time: '40分钟', course: '数据库系统', reason: '拓展相关知识领域' },
  ],
}
</script>
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="result" class="space-y-6">
      ...
      <!-- 智能推荐资源 -->
      <div v-if="result.recommended_resources && result.recommended_resources.length > 0" class="bg-white rounded-2xl shadow-lg p-6">
        <h3 class="text-lg font-bold text-gray-800 mb-4">🎯 为你推荐的学习资源</h3>
        <p class="text-sm text-gray-600 mb-4">根据你的测评结果和学习画像，我们为你生成了个性化的学习建议：</p>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div
            v-for="item in result.recommended_resources"
            :key="item.id"
            class="flex items-start gap-4 p-4 rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 hover:shadow-md transition-all"
          >
            ...
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span :class="[
                  ...
                  {{ item.priority === 'high' ? '重点' : item.priority === 'medium' ? '推荐' : '一般' }}
                </span>
              </div>
            ...
            <button class="px-4 py-2 bg-amber-500 text-white text-sm font-medium rounded-lg hover:bg-amber-600 transition-all">
              开始学习 →
            </button>
          </div>
        </div>
